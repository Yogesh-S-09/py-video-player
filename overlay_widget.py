# overlay_widget.py
# (UPDATED with seek buttons and corrected chapter icons)
...
class OverlayWidget(QWidget):
    ...
    def create_widgets(self):
        ...
        # --- ICONS CORRECTED & NEW BUTTONS ADDED ---
        self.prev_chapter_btn = QPushButton()
        self.prev_chapter_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_MediaSkipBackward)) # Correct icon
        
        self.seek_backward_btn = QPushButton("-10s") # New button
        self.seek_backward_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_MediaSeekBackward))
        
        self.seek_forward_btn = QPushButton("+10s") # New button
        ...
        self.next_chapter_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_MediaSkipForward)) # Correct icon
        # -------------------------------------------
    ...
    def create_layouts(self):
        ...
        # --- BOTTOM LAYOUT UPDATED ---
        bottom_layout = QHBoxLayout()
        bottom_layout.addWidget(self.play_pause_btn)
        bottom_layout.addWidget(self.prev_chapter_btn)
        bottom_layout.addWidget(self.seek_backward_btn) # New
        bottom_layout.addWidget(self.seek_forward_btn)  # New
    ...
    def update_chapter_menu(self, chapters):
        ...
        self.prev_chapter_btn.setEnabled(True)
        self.next_chapter_btn.setEnabled(True)
        for i, chap in enumerate(self.chapter_list):
            title = chap.get('title', f"Chapter {i+1}")
            start_time = chap.get('time', 0)
